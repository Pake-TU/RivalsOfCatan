...
public class ExpansionCardEffectHandler {
    ...
    public static boolean applyExpansionEffect(Card card, Player active, Player other, int row, int col) {
        ...
        if ("Building".equalsIgnoreCase(card.type)) {
            return applyBuildingEffect(card, active, row, col);
        }
        
        // Units
        if (card.type != null && card.type.contains("Unit")) {
            return applyUnitEffect(card, active, row, col);
        }
        
        return false;
    }
    
    private static boolean applyBuildingEffect(Card card, Player active, int row, int col) {
        String name = card.name == null ? "" : card.name;
        ...
        addCardStats(card, active);
        return true;
    }
    
    private static boolean applyUnitEffect(Card card, Player active, int row, int col) {
        String name = card.name == null ? "" : card.name;
     ...
     */
    private static void addCardStats(Card card, Player player) {
